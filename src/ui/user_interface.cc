...
void UserInterface::SetupHomePage() {
    ...
    play_button->onClick([this] {
        SwitchState(UserInterfaceState::LevelSelection);
    });
...
void UserInterface::SetupLevelSelection() {
    ...
    for (const auto& level: available_levels_) {
        ...
        if (selected_level_ && selected_level_ == level) {
            ...
            selectButton->getRenderer()->setBackgroundColor(tgui::Color(220, 220, 220));
    ...
    backButton->onClick([this] {
        SwitchState(UserInterfaceState::HomePage);
    });
    gui_.add(backButton);
...
void UserInterface::SetupGame() {
    ...
    // Level name
    auto levelLabel = tgui::Label::create("Playing level: " + selected_level_->name());
    ...
    // Category name
    auto worldLabel = tgui::Label::create("World: " + selected_level_->category());
    ...
    auto exitButton = tgui::Button::create("Exit");
    exitButton->setSize(200, 60);
    exitButton->setPosition("(&.width) - 220", "(&.height) - 80"); // bottom right corner
    exitButton->onClick([this] {
        window_.close();
    });
    gui_.add(exitButton);

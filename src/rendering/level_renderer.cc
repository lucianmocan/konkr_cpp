...
namespace konkr
{
    void LevelRenderer::Render(sf::RenderTarget& target, const Level& level, float hex_radius) const {
        ...
        const float map_width = num_cols * hex_width + (hex_width / 2); // Account for indentation
        ...
        for (size_t row = 0; row < tiles.size(); ++row) {
            ...
            for (size_t col = 0; col < tile_row.size(); ++col) {
                ...
                float x = col * hex_width + (indent ? (hex_width / 2) : 0) + x_padding + x_offset;
